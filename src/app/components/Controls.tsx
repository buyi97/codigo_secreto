      ...
      <div className="flex-1 space-y-4 overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="space-y-2.5">
          ...
          <div className="space-y-1.5 pl-1">
            {redPlayers.map(p => (
              <PlayerRow 
                key={p.id} player={p} isMe={p.id === currentPlayer?.id} 
                isHost={isHost || false} onSelectRole={onSelectRole}
                onMovePlayer={onMovePlayer} gameStatus={gameStatus}
              />
            ))}
          </div>
        </div>
                ...
                <div key={p.id} className="flex items-center justify-between py-1 group">
                  ...
                  {isHost && gameStatus !== "playing" && (
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => onMovePlayer(p.id, "red")} className="text-[10px] uppercase font-bold text-red-team/60 hover:text-red-team">Rojo</button>
                      <button onClick={() => onMovePlayer(p.id, "blue")} className="text-[10px] uppercase font-bold text-blue-team/60 hover:text-blue-team">Azul</button>
                    </div>
                  )}
                </div>
              ))}
...
const PlayerRow: React.FC<{ 
  player: Player, isMe: boolean, isHost: boolean,
  onSelectRole: (role: Role) => void,
  onMovePlayer: (playerId: string, team?: PlayerTeam, role?: Role) => void,
  gameStatus: string
  ...
  return (
    <div className="flex items-center justify-between group py-2 border-b border-white/5 last:border-0">
      <div className="flex items-center gap-2 text-xs md:text-sm font-bold text-text-main truncate pr-2">
        ...
        <span className={cn("truncate tracking-tight", isMe && "text-white underline underline-offset-4 decoration-2 decoration-blue-team")}>{player.name}</span>
        {player.isHost && <Crown className="w-3.5 h-3.5 text-amber-400 shrink-0" />}
      </div>
      
      <div className="flex items-center gap-2 shrink-0">
        <span className={cn("text-[9px] md:text-[10px] uppercase font-black tracking-widest px-2 py-0.5 rounded-md border", player.role === "spymaster" ? "text-amber-400 bg-amber-400/10 border-amber-400/20" : "text-neutral-team/60 bg-white/5 border-white/5")}>
          {player.role === "spymaster" ? "LÍDER" : "AGENTE"}
        </span>
        
        <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
          {isMe && canChange && (
            <button onClick={() => onSelectRole(player.role === "operative" ? "spymaster" : "operative")} className="text-[10px] uppercase font-bold text-blue-team hover:text-blue-team/80 transition-colors">Cambiar Rol</button>
          )}
          {isHost && canChange && !isMe && (
            <>
              <button onClick={() => onMovePlayer(player.id, player.team === "red" ? "blue" : "red")} className="text-[10px] uppercase font-bold text-white/30 hover:text-white transition-colors" title="Cambiar Equipo">Equipo</button>
              <button onClick={() => onMovePlayer(player.id, undefined, player.role === "operative" ? "spymaster" : "operative")} className="text-[10px] uppercase font-bold text-white/30 hover:text-white transition-colors" title="Cambiar Rol">Rol</button>
            </>
          )}
        </div>
      </div>
