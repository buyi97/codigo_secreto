...
export default function GameRoom({ gameState, socket, currentPlayer, onLeave }: GameRoomProps) {
  ...
  // FIX CLAVE: Algunas versiones del backend mandan "finished" y otras "ended".
  const isGameFinished = gameState.status === "finished" || gameState.status === "ended";

  // Volvemos todos a la vista de juego cuando el estado sea "playing"
  useEffect(() => {
  ...
  const handleVolverAlCuartel = () => {
    // Todos se van al cuartel localmente. El ADMIN reinicia para todos cuando hace clic en iniciar.
  ...
  // Cálculos de Cartas y Marcadores
  ...
  return (
    <div className="flex flex-col h-[100dvh] overflow-hidden bg-bg relative">
      ...
      {/* ================= BARRA SUPERIOR (HORIZONTAL ENORME) ================= */}
      {/* Se agrandó el padding (py-4), la altura base y el tamaño general de los elementos en md: y lg: */}
      <header className="w-full flex flex-wrap md:flex-nowrap items-center justify-between bg-black/90 border-b border-white/10 shrink-0 z-20 p-2 md:px-6 md:h-28 gap-2">
        ...
        <div className="flex items-center justify-between md:justify-start w-full md:w-auto gap-2 shrink-0">
          <div className="flex items-center gap-1 md:gap-3 bg-white/5 px-2 md:px-4 py-1 md:py-2.5 rounded-xl border border-white/10">
            <span className="text-[10px] md:text-sm uppercase tracking-widest text-neutral-team font-black mr-1 hidden sm:inline">Historial</span>
            <span className={cn("font-black transition-all text-sm md:text-2xl", history.red > history.blue ? "text-[#FF4B4B]" : "text-[#FF4B4B]/70")}>{history.red}</span>
            <span className="text-white/20 text-xs md:text-lg mx-0.5">-</span>
            <span className={cn("font-black transition-all text-sm md:text-2xl", history.blue > history.red ? "text-[#4B9FFF]" : "text-[#4B9FFF]/70")}>{history.blue}</span>
          </div>
          
          <div className="flex items-center gap-1.5 bg-black/50 text-white px-2 md:px-4 py-1.5 md:py-2.5 rounded-xl border border-white/10">
            <span className="text-[10px] md:text-sm text-neutral-team uppercase font-bold hidden sm:inline">Sala</span>
            <span className="font-mono text-sm md:text-xl font-bold text-primary">{gameState.roomId}</span>
        ...
        {effectiveStatus !== "lobby" && (
          <div className="w-full md:flex-1 flex items-center justify-between md:justify-center order-3 md:order-2 gap-2 mt-2 md:mt-0">
            
            {/* ESTADO FINAL: Reemplaza a los marcadores en la barra superior */}
            {isGameFinished ? (
              <div className="flex-1 flex flex-col items-center justify-center animate-in zoom-in duration-500 py-1">
                <div className="flex items-center gap-2 md:gap-4">
                  <Trophy className={cn("w-5 h-5 md:w-8 md:h-8 lg:w-10 lg:h-10", gameState.winner === "red" ? "text-[#FF4B4B]" : "text-[#4B9FFF]")} />
                  <h2 className="text-sm md:text-2xl lg:text-3xl font-black uppercase text-white tracking-widest leading-none">
                    Misión Completada
                  </h2>
                  <Trophy className={cn("w-5 h-5 md:w-8 md:h-8 lg:w-10 lg:h-10", gameState.winner === "red" ? "text-[#FF4B4B]" : "text-[#4B9FFF]")} />
                </div>
                <span className={cn("text-[10px] md:text-sm lg:text-lg font-bold uppercase tracking-widest mt-1 md:mt-2", gameState.winner === "red" ? "text-[#FF4B4B]" : "text-[#4B9FFF]")}>
                  ¡Victoria del Equipo {gameState.winner === "red" ? "Rojo" : "Azul"}!
                </span>
              </div>
            ) : (
              /* ESTADO JUGANDO: Marcadores Distribuidos a lo Ancho */
              <>
                {/* Marcador Rojo */}
                <div className="flex-1 flex justify-end md:justify-center">
                  <div className="flex items-baseline gap-0.5 md:gap-2 px-3 md:px-6 py-1.5 md:py-2.5 rounded-xl bg-[#FF4B4B]/10 border border-[#FF4B4B]/30 shadow-inner">
                    <span className="text-lg md:text-3xl lg:text-4xl font-black text-[#FF4B4B] leading-none">{redRemaining}</span>
                    <span className="text-xs md:text-sm lg:text-base font-bold text-[#FF4B4B]/60 leading-none">/{redTotal}</span>
                  </div>
                </div>

                {/* Central Info (Turno Explícito + Timer) */}
                <div className="flex flex-col items-center justify-center shrink-0 md:px-8 lg:px-16">
                  <div className="text-[9px] md:text-sm lg:text-base font-black uppercase tracking-widest leading-none mb-1 md:mb-2">
                    TURNO {turnPhase} <span className={turnTeamColor}>{turnTeamName}</span>
                  </div>
                  <div className={cn(
                    "px-3 py-0.5 md:px-6 md:py-1.5 rounded-md md:rounded-xl border font-mono text-sm md:text-2xl lg:text-3xl font-bold transition-all shadow-md leading-tight",
                ...
                <div className="flex-1 flex justify-start md:justify-center">
                  <div className="flex items-baseline gap-0.5 md:gap-2 px-3 md:px-6 py-1.5 md:py-2.5 rounded-xl bg-[#4B9FFF]/10 border border-[#4B9FFF]/30 shadow-inner">
                    <span className="text-lg md:text-3xl lg:text-4xl font-black text-[#4B9FFF] leading-none">{blueRemaining}</span>
                    <span className="text-xs md:text-sm lg:text-base font-bold text-[#4B9FFF]/60 leading-none">/{blueTotal}</span>
                  </div>
                </div>
              </>
            )}
        ...
        <div className="flex items-center gap-1 md:gap-3 order-2 md:order-3 shrink-0">
          {gameState.status === "playing" && (
             <button onClick={() => setShowTeamsPanel(true)} className="p-1.5 md:px-4 md:py-2.5 rounded-xl border border-white/20 bg-white/5 hover:bg-white/10 text-white flex items-center gap-2 transition-all shadow-lg">
               <Users className="w-4 h-4 md:w-6 md:h-6" /> <span className="hidden xl:inline text-base font-bold">Equipos</span>
             </button>
          )}
          <button onClick={() => setShowLeaveConfirm(true)} className="p-1.5 md:p-3 hover:bg-[#FF4B4B]/20 rounded-xl transition-colors border border-white/20 backdrop-blur-md group">
            <LogOut className="w-4 h-4 md:w-6 md:h-6 text-neutral-team group-hover:text-[#FF4B4B]" />
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        {effectiveStatus !== "lobby" && (
          <div className="flex-1 flex flex-col lg:flex-row min-h-0 w-full relative">
             ...
             {/* COLUMNA CENTRAL (Tablero + Botonera de Acción Fija) */}
             <div className="flex-1 flex flex-col min-w-0 min-h-0 relative">
                
                {/* Contenedor del Tablero: El tablero usa flex 100% y se adapta */}
                <div className="flex-1 w-full min-h-0 flex items-stretch justify-center p-1.5 md:p-4">
                   <div className={cn(
                     "w-full max-w-6xl h-full flex transition-opacity duration-700",
                     isGameFinished && "pointer-events-none opacity-100" 
                   )}>
                     <Board cards={gameState.cards} isSpymaster={effectiveIsSpymaster} onCardClick={handleCardClick} canClick={canClick} />
                   </div>
                </div>

                {/* BOTONERA FIJA INFERIOR (Nunca queda tapada) */}
                <div className="shrink-0 p-3 md:p-4 border-t border-white/10 bg-bg/95 backdrop-blur-xl flex flex-wrap items-center justify-center gap-3 md:gap-4 z-20 shadow-[0_-10px_30px_rgba(0,0,0,0.6)] pointer-events-auto">
                  {isGameFinished ? (
                    // BOTÓN VOLVER AL LOBBY PARA TODOS
                    <button onClick={handleVolverAlCuartel} className="w-full md:max-w-md py-3 md:py-4 px-6 md:px-8 bg-primary hover:bg-primary-hover text-white font-black text-sm md:text-lg rounded-xl uppercase tracking-widest shadow-[0_0_20px_rgba(75,159,255,0.4)] transition-all flex items-center justify-center gap-2">
                        <RotateCcw className="w-5 h-5 md:w-6 md:h-6" /> Volver al lobby de sala
                    </button>
                  ) : (
                    ...
                    <>
                      ...
                      {((!isSpymaster && isMyTurn) || (isSinglePlayer && gameState.currentClue)) && (
                        <button onClick={handleEndTurn} className="flex-1 md:flex-none md:px-12 py-2.5 md:py-3 bg-white text-bg rounded-xl text-sm md:text-base font-black uppercase tracking-widest hover:bg-white/90 shadow-[0_0_15px_rgba(255,255,255,0.3)] transition-all active:scale-95">
                          Pasar Turno
                        </button>
                      )}
             ...
             {/* SIDEBAR LATERAL (PC) / PANEL INFERIOR (MÓVIL) - Registro de Inteligencia */}
             <aside className={cn(
                "w-full lg:w-80 xl:w-96 shrink-0 flex flex-col bg-bg-dark border-t lg:border-t-0 lg:border-l border-white/10 z-20 shadow-[0_-5px_20px_rgba(0,0,0,0.5)] lg:shadow-none transition-transform duration-300",
                ...
                isGameFinished && "hidden lg:flex opacity-30 pointer-events-none" // En mobile se oculta entero si terminó para no tapar tablero
             )}>
                ...
                <div className="hidden lg:block p-4 border-b border-white/10 bg-black/20 text-center text-xs font-bold uppercase tracking-widest text-neutral-team shrink-0">
                  Registro de Inteligencia
                </div>
                
                <div className="flex-1 overflow-y-auto p-2 lg:p-4 custom-scrollbar bg-black/10 min-h-0">
                  <ClueHistory clues={gameState.clues || []} />
                </div>

                {gameState.currentClue && !isGameFinished && (
                  <div className="shrink-0 p-2 lg:p-4 bg-black/40 border-t border-white/10 flex items-center justify-between shadow-inner">
                    <div className="flex flex-col">
                      ...
                      <span className="text-sm md:text-lg font-black text-white">{gameState.currentClue.word} <span className="text-primary">({gameState.currentClue.count})</span></span>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="text-[9px] md:text-[10px] uppercase text-neutral-team font-bold">Adivinadas</span>
                      <span className="text-sm md:text-lg font-bold text-white">{gameState.guessesMade} / {gameState.currentClue.count === 0 ? "∞" : gameState.currentClue.count}</span>
                    </div>
                  </div>
                )}
